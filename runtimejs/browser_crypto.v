...
fn cli_browser_crypto_boot(ctx &vjsx.Context, boot vjsx.Value) {
	...
	obj.set('pbkdf2_sha256', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		bytes := cli_browser_pbkdf2_sha256(args[0].to_bytes(), args[1].to_bytes(), args[2].to_int(), args[3].to_int()) or { return ctx.js_throw(err.msg()) }
		return ctx.js_array_buffer(bytes)
	}))
	obj.set('pbkdf2_sha384', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		bytes := cli_browser_pbkdf2_sha384(args[0].to_bytes(), args[1].to_bytes(), args[2].to_int(), args[3].to_int()) or { return ctx.js_throw(err.msg()) }
		return ctx.js_array_buffer(bytes)
	}))
	obj.set('pbkdf2_sha512', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		bytes := cli_browser_pbkdf2_sha512(args[0].to_bytes(), args[1].to_bytes(), args[2].to_int(), args[3].to_int()) or { return ctx.js_throw(err.msg()) }
		return ctx.js_array_buffer(bytes)
	}))
	obj.set('ecdsa_generate_key', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		nid := cli_browser_ecdsa_nid_from_curve_name(args[0].to_string()) or {
			return ctx.js_throw(err.msg())
		}
	...
	obj.set('ecdsa_public_from_private', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		nid := cli_browser_ecdsa_nid_from_curve_name(args[1].to_string()) or {
			return ctx.js_throw(err.msg())
		}
	...
	obj.set('ecdsa_sign', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		nid := cli_browser_ecdsa_nid_from_curve_name(args[2].to_string()) or {
			return ctx.js_throw(err.msg())
		}
		opt := cli_browser_ecdsa_signer_opts_from_hash_name(args[3].to_string()) or {
			return ctx.js_throw(err.msg())
		}
		privatekey := ecdsa.new_key_from_seed(args[0].to_bytes(), nid: nid) or {
			return ctx.js_throw(err.msg())
	...
	obj.set('ecdsa_verify_with_private', ctx.js_function(fn [ctx] (args []vjsx.Value) vjsx.Value {
		nid := cli_browser_ecdsa_nid_from_curve_name(args[3].to_string()) or {
			return ctx.js_bool(false)
		}
		opt := cli_browser_ecdsa_signer_opts_from_hash_name(args[4].to_string()) or {
			return ctx.js_bool(false)
		}
		privatekey := ecdsa.new_key_from_seed(args[0].to_bytes(), nid: nid) or {
			return ctx.js_bool(false)
