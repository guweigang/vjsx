...
fn sha384_hash(data []u8) []u8 {
	return sha512.sum384(data)
}

fn sha512_hash(data []u8) []u8 {
...
fn crypto_boot(ctx &Context, boot Value) {
	...
	obj.set('hmac_sha1', ctx.js_function(fn [ctx] (args []Value) Value {
		sum := hmac.new(args[0].to_bytes(), args[1].to_bytes(), sha1_hash, sha1.block_size)
		return ctx.js_array_buffer(sum)
	}))
	obj.set('hmac_sha256', ctx.js_function(fn [ctx] (args []Value) Value {
		sum := hmac.new(args[0].to_bytes(), args[1].to_bytes(), sha256_hash, sha256.block_size)
		return ctx.js_array_buffer(sum)
	}))
	obj.set('hmac_sha384', ctx.js_function(fn [ctx] (args []Value) Value {
		sum := hmac.new(args[0].to_bytes(), args[1].to_bytes(), sha384_hash, sha512.block_size)
		return ctx.js_array_buffer(sum)
	}))
	obj.set('hmac_sha512', ctx.js_function(fn [ctx] (args []Value) Value {
		sum := hmac.new(args[0].to_bytes(), args[1].to_bytes(), sha512_hash, sha512.block_size)
		return ctx.js_array_buffer(sum)
	}))
	obj.set('timing_safe_equal', ctx.js_function(fn [ctx] (args []Value) Value {
	...
	obj.set('ed25519_public_from_private', ctx.js_function(fn [ctx] (args []Value) Value {
