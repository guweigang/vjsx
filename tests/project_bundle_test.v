...
fn test_project_bundle_transpiles_typescript_and_json_without_runtime_sources() {
	...
	os.write_file(os.join_path(root, 'main.mts'), 'import { add } from "./dep.ts"; import data from "./data.json"; export const result = add(data.value, 2);') or {
		panic(err)
	}
	os.write_file(os.join_path(root, 'dep.ts'), 'export function add(a: number, b: number): number { return a + b; }') or { panic(err) }
...
fn test_project_bundle_preserves_commonjs_require_and_exports() {
	...
	os.rmdir_all(root) or {}
	os.mkdir_all(root) or { panic(err) }
	os.write_file(os.join_path(root, 'main.cjs'), 'const helper = require("./helper.cjs"); module.exports = { result: helper.add(40, 2) };') or {
		panic(err)
	}
	os.write_file(os.join_path(root, 'helper.cjs'), 'module.exports = { add(a, b) { return a + b; } };') or { panic(err) }
...
fn test_project_bundle_includes_static_node_modules_dependencies() {
	...
	os.write_file(os.join_path(root, 'main.mts'), 'import { answer } from "answer-package"; export const result = answer;') or { panic(err) }
	os.write_file(os.join_path(package_root, 'package.json'), '{"name":"answer-package","version":"1.0.0","type":"module","exports":"./index.js"}') or {
		panic(err)
	}
