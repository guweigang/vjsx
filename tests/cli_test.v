...
fn test_cli_example_db_sqlite_basic() {
	...
	os.rm(db_path) or {}
	output :=
		os.execute('VJS_V_FLAGS="-d vjsx_sqlite" sh ./vjsx --module ./examples/db/sqlite_basic.mjs')
	assert output.exit_code == 0
...
fn test_cli_host_sqlite_runtime_features() {
	...
	os.rm(db_path) or {}
	output :=
		os.execute('VJS_V_FLAGS="-d vjsx_sqlite" sh ./vjsx --module ./tests/host_sqlite_runtime.mjs')
	assert output.exit_code == 0
...
fn test_cli_host_sqlite_transaction_helper() {
	...
	os.rm(db_path) or {}
	output :=
		os.execute('VJS_V_FLAGS="-d vjsx_sqlite" sh ./vjsx --module ./tests/host_sqlite_transaction_runtime.mjs')
	assert output.exit_code == 0
	assert output.output.trim_space() == 'false\ntrue\nfalse\n2\nrollback-me\nalice,bob'
	assert !os.exists(db_path)
}
...
fn test_cli_host_sqlite_statement_helper() {
	...
	os.rm(db_path) or {}
	output :=
		os.execute('VJS_V_FLAGS="-d vjsx_sqlite" sh ./vjsx --module ./tests/host_sqlite_statement_runtime.mjs')
	assert output.exit_code == 0
...
fn test_cli_host_sqlite_close_lifecycle() {
	...
	os.rm(db_path) or {}
	output :=
		os.execute('VJS_V_FLAGS="-d vjsx_sqlite" sh ./vjsx --module ./tests/host_sqlite_close_runtime.mjs')
	assert output.exit_code == 0
...
fn test_cli_host_error_runtime_features() {
	...
	os.rm(db_path) or {}
	output :=
		os.execute('VJS_V_FLAGS="-d vjsx_sqlite" sh ./vjsx --module ./tests/host_error_runtime.mjs')
	assert output.exit_code == 0
