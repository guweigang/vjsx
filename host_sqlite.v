...
$if vjsx_sqlite ? {
	...
	fn sql_stmt_kind(query_text string) string {
		...
		if head.starts_with('insert ') || head.starts_with('update ') || head.starts_with('delete ')
			|| head.starts_with('replace ') || head.starts_with('create ')
			|| head.starts_with('drop ') || head.starts_with('alter ') || head.starts_with('begin ')
	...
	fn sqlite_params(args []vjsx.Value, index int) ![]string {
	...
	fn sqlite_param_batches(args []vjsx.Value, index int) ![][]string {
		if index >= args.len || args[index].is_undefined() || args[index].is_null() {
			return [][]string{}
		}
		if !args[index].is_array() {
	...
	fn sqlite_stmt_object(ctx &Context, mut stmt HostSqliteStmt) Value {
		...
		to_string_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
			return ctx.js_string('sqlite.Statement{kind: ${sql_stmt_kind(stmt.query_text)}, closed: ${stmt.closed}, sql: ${stmt.query_text}}')
		})
		query_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
		...
		query_one_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if stmt.closed {
				query_err = sqlite_error_value(ctx, 'sqlite statement is closed', 'Error')
		...
		scalar_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if stmt.closed {
				query_err = sqlite_error_value(ctx, 'sqlite statement is closed', 'Error')
		...
		query_many_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if stmt.closed {
				query_err = sqlite_error_value(ctx, 'sqlite statement is closed', 'Error')
		...
		exec_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
		...
		exec_many_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
			mut exec_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if stmt.closed {
				exec_err = sqlite_error_value(ctx, 'sqlite statement is closed', 'Error')
		...
		close_fn := ctx.js_function(fn [ctx, mut stmt] (args []vjsx.Value) Value {
			promise := ctx.js_promise()
			sqlite_mark_stmt_closed(mut stmt)
	...
	fn sqlite_conn_object(ctx &Context, mut conn HostSqliteConn) Value {
		...
		to_string_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			return ctx.js_string('sqlite.Connection{path: ${conn.path}, closed: ${conn.closed}, inTransaction: ${conn.in_tx}}')
		})
		query_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				query_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
		...
		query_one_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				query_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
		...
		scalar_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				query_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
		...
		query_many_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut query_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				query_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
		...
		exec_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut exec_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				exec_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
		...
		exec_many_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut exec_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				exec_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
		...
		close_fn := ctx.js_function(fn [ctx, mut conn, obj] (args []vjsx.Value) Value {
		...
		begin_fn := ctx.js_function(fn [ctx, mut conn, obj] (args []vjsx.Value) Value {
		...
		commit_fn := ctx.js_function(fn [ctx, mut conn, obj] (args []vjsx.Value) Value {
		...
		rollback_fn := ctx.js_function(fn [ctx, mut conn, obj] (args []vjsx.Value) Value {
		...
		transaction_fn := ctx.js_function(fn [ctx, mut conn, obj] (args []vjsx.Value) Value {
			...
			if callback_result.instanceof('Promise') {
				ignore_rejection := ctx.js_function(fn [ctx] (args []vjsx.Value) Value {
		...
		prepare_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
		...
		prepare_cached_fn := ctx.js_function(fn [ctx, mut conn] (args []vjsx.Value) Value {
			mut prepare_err := ctx.js_undefined()
			promise := ctx.js_promise()
			if conn.closed {
				prepare_err = sqlite_error_value(ctx, 'sqlite connection is closed', 'Error')
	...
	pub fn (ctx &Context) install_sqlite_module(roots []string) {
		...
		open_fn := ctx.js_function(fn [ctx, roots] (args []vjsx.Value) Value {
